#!/usr/bin/env python3
"""Builds a single-file, Notion-styled, RTL Modern Standard Arabic HTML edition of the book from book/chapters-msa/*.md."""
import re
import html
import markdown
...
CHAPTERS_DIR = ROOT / "chapters-msa"
OUT_HTML = Path(__file__).resolve().parent / "index.html"

# (filename stem, part label or None, emoji icon)
STRUCTURE = [
    ("00-preface", None, "👋"),
    ("01-why-start-a-startup", "الجزء الأول — لماذا وماذا: الفكرة", "🚀"),
    ("02-anatomy-of-a-great-idea", "الجزء الأول — لماذا وماذا: الفكرة", "💡"),
    ("03-monopoly-not-competition", "الجزء الأول — لماذا وماذا: الفكرة", "👑"),
    ("04-choosing-a-cofounder", "الجزء الثاني — من: الـ Founders والفريق", "🤝"),
    ("05-the-first-ten-hires", "الجزء الثاني — من: الـ Founders والفريق", "🧑‍💼"),
    ("06-designing-company-culture", "الجزء الثاني — من: الـ Founders والفريق", "🎨"),
    ("07-idea-to-mvp-talking-to-users", "الجزء الثالث — ماذا: بناء المنتج", "🗣️"),
    ("08-building-products-users-love", "الجزء الثالث — ماذا: بناء المنتج", "❤️"),
    ("09-doing-things-that-dont-scale", "الجزء الثالث — ماذا: بناء المنتج", "🏃"),
    ("09b-interlude-hardware-in-a-software-world", "الجزء الثالث — ماذا: بناء المنتج", "🔌"),
    ("10-retention-is-the-root-of-growth", "الجزء الرابع — النمو (Growth)", "📈"),
    ("11-growth-channels-and-tactics", "الجزء الرابع — النمو (Growth)", "🌱"),
    ("12-getting-press", "الجزء الرابع — النمو (Growth)", "📰"),
    ("13-how-to-raise-money", "الجزء الخامس — المال", "💰"),
    ("14-pitching-and-fundraising-mechanics", "الجزء الخامس — المال", "🎤"),
    ("15-legal-and-financial-mechanics", "الجزء الخامس — المال", "⚖️"),
    ("16-selling-first-customer-to-enterprise-deal", "الجزء الخامس — المال", "🛒"),
    ("17-execution-focus-speed-momentum", "الجزء السادس — التنفيذ والقيادة والتوسع", "⚡"),
    ("18-the-founders-mind-paradox-and-judgment", "الجزء السادس — التنفيذ والقيادة والتوسع", "🧠"),
    ("19-how-to-operate", "الجزء السادس — التنفيذ والقيادة والتوسع", "🛠️"),
    ("20-how-to-manage", "الجزء السادس — التنفيذ والقيادة والتوسع", "👔"),
    ("21-scaling-the-company", "الجزء السادس — التنفيذ والقيادة والتوسع", "🏗️"),
    ("22-epilogue-the-long-road", None, "🌅"),
    ("23-appendix-a-cap-table-walkthrough", "الملاحق", "📊"),
    ("24-appendix-b-term-sheet-primer", "الملاحق", "📄"),
    ("25-appendix-c-unit-economics-formulas", "الملاحق", "🧮"),
    ("26-appendix-d-full-pitch-roleplay-transcript", "الملاحق", "🎭"),
    ("27-glossary", "مصادر الكتاب", "📖"),
    ("28-bibliography", "مصادر الكتاب", "📚"),
    ("29-index", "مصادر الكتاب", "🔎"),
]

# heading text (substring match) -> (emoji, css class) — matches the fixed MSA
# translations used consistently across every chapter during translation.
SECTION_STYLES = [
    ("الفكرة الأساسية", "💡", "callout-blue"),
    ("لماذا هذا صحيح", "🔍", "callout-teal"),
    ("أخطاء شائعة", "⚠️", "callout-red"),
    ("المقايضات والحالات الخاصة", "⚖️", "callout-purple"),
    ("التطبيق العملي", "🧪", "callout-green"),
    ("خلاصة الفصل", "✅", "callout-yellow"),
    ("تمارين", "📝", "callout-pink"),
    ("لمزيد من القراءة", "📚", "callout-gray"),
]
...
def slugify(text, fallback):
    s = re.sub(r"[^a-z0-9-]", "", re.sub(r"\s+", "-", text.strip().lower()))
...
def wrap_sections(body_html, chapter_slug):
    ...
    while i < len(parts):
        ...
        counter += 1
        anchor = f"{chapter_slug}-s{counter}-{slugify(text, 'section')}"
        if css:
...
<head>
<meta charset="UTF-8">
<title>كيف تبدأ Startup — الكتاب</title>
...
body{
  margin:0; display:flex; min-height:100vh; direction:rtl; text-align:right;
  font-family:"SF Arabic","Geeza Pro",-apple-system,BlinkMacSystemFont,"Segoe UI",Tahoma,Arial,sans-serif;
  color:var(--text); background:var(--bg); line-height:1.85;
}
/* Sidebar (visually on the right, since body direction is rtl) */
nav.sidebar{
...
/* Main content */
main{flex:1; min-width:0; padding:56px 48px 120px; max-width:860px; margin:0 auto;}
...
.chapter-title{font-size:2.1em; font-weight:800; margin:0 0 28px; letter-spacing:0;}
h2{font-size:1.5em; font-weight:750; margin:0 0 14px; display:flex; align-items:center; gap:8px;}
h3{font-size:1.15em; font-weight:700; margin:28px 0 10px; color:#1a5c7a; border-right:3px solid var(--blue-border); padding-right:10px;}
p{margin:0 0 16px;}
strong{color:#2b2926; font-weight:700;}
a{color:#0b6e99;}
ul,ol{padding-right:1.4em; padding-left:0; margin:0 0 16px;}
li{margin-bottom:6px;}
li::marker{color:#c48d3a;}
blockquote{
  margin:20px 0; padding:14px 18px; background:#f7f6f3; border-right:4px solid #d9a441;
  border-left:none; border-radius:4px; font-style:italic; color:#4b4a45;
}
...
pre{
  ...
  line-height:1.5; margin:20px 0; direction:ltr; text-align:left;
}
...
@media (max-width:900px){
  ...
  nav.sidebar{width:100%; height:auto; position:relative; border-left:none; border-bottom:1px solid var(--border);}
...
<body>
<nav class="sidebar">
  <div class="sidebar-title">📘 كيف تبدأ Startup</div>
  {{SIDEBAR}}
</nav>
<main>
{{CONTENT}}
